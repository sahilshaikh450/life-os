...
const BASE_URL = process.env.REACT_APP_API_URL || 'https://life-os-production-8bc2.up.railway.app';
...
export const habitApi = {
  ...
  getOne: (id) => api.get(`/api/habits/${id}`),
  create: (data) => api.post('/api/habits', data),
  update: (id, data) => api.put(`/api/habits/${id}`, data),
  delete: (id) => api.delete(`/api/habits/${id}`),
  archive: (id) => api.patch(`/api/habits/${id}/archive`),
  log: (id, data) => api.post(`/api/habits/${id}/log`, data),
  getLogs: (id, start, end) => api.get(`/api/habits/${id}/logs`, { params: { start, end } }),
...
export const expenseApi = {
  ...
  getByRange: (start, end) => api.get('/api/expenses/range', { params: { start, end } }),
  create: (data) => api.post('/api/expenses', data),
  update: (id, data) => api.put(`/api/expenses/${id}`, data),
  delete: (id) => api.delete(`/api/expenses/${id}`),
  getStats: () => api.get('/api/expenses/stats'),
  createBudget: (data) => api.post('/api/expenses/budgets', data),
  getBudgets: (month, year) => api.get(`/api/expenses/budgets`, { params: { month, year } }),
  deleteBudget: (id) => api.delete(`/api/expenses/budgets/${id}`),
};
...
export const todoApi = {
  ...
  getByStatus: (status) => api.get(`/api/todos/status/${status}`),
  ...
  createProject: (data) => api.post('/api/todos/projects', data),
  getProjects: () => api.get('/api/todos/projects'),
  deleteProject: (id) => api.delete(`/api/todos/projects/${id}`),
};
