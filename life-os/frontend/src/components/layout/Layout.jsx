...
export default function Layout() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: '#070714', overflow: 'hidden', fontFamily: "'Sora', sans-serif" }}>
      {/* Sidebar */}
      <motion.aside
      ...
      >
        {/* Logo */}
        <div style={{ padding: '24px 20px', borderBottom: '1px solid rgba(167,139,250,0.1)', display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
          ...
          <AnimatePresence>
            {sidebarOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#fff', letterSpacing: '-0.5px' }}>Life OS</div>
        ...
        <button
          ...
          style={{
            ...
            background: '#1e1e3a', border: '1px solid rgba(167,139,250,0.3)',
            borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
            cursor: 'pointer', color: '#a78bfa', zIndex: 20,
          }}
        >
        ...
        <nav style={{ flex: 1, padding: '16px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
          <AnimatePresence>
            {sidebarOpen && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                style={{ fontSize: 10, color: '#4b5563', letterSpacing: '0.15em', padding: '8px 8px 4px', fontWeight: 600 }}>
                NAVIGATION
              </motion.div>
            )}
          </AnimatePresence>
            ...
            >
              {({ isActive }) => (
                ...
                >
                  ...
                  <Icon size={18} color={isActive ? color : '#6b7280'} style={{ flexShrink: 0 }} />
                  <AnimatePresence>
                    {sidebarOpen && (
                      <motion.span
                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        style={{ fontSize: 14, fontWeight: isActive ? 600 : 400, color: isActive ? '#f1f5f9' : '#9ca3af', whiteSpace: 'nowrap' }}
                      >
                        {label}
                      </motion.span>
                    )}
                  </AnimatePresence>
                  {isActive && sidebarOpen && <ChevronRight size={14} color={color} style={{ marginLeft: 'auto' }} />}
                </motion.div>
              )}
            </NavLink>
          ))}
      ...
      <main style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        {/* Top bar */}
        <div style={{
          padding: '16px 28px', borderBottom: '1px solid rgba(167,139,250,0.1)',
          ...
          <button style={{
            ...
            borderRadius: 8, padding: '6px 12px', color: '#a78bfa', cursor: 'pointer', fontSize: 13,
          }}>
            <Bell size={15} style={{ verticalAlign: 'middle' }} />
          </button>
          <button style={{
            background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.2)',
            borderRadius: 8, padding: '6px 12px', color: '#a78bfa', cursor: 'pointer', fontSize: 13,
